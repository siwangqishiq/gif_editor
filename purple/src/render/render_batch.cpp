...
namespace purple{
    void Batch::allocatorMemory(){
        ...
        Log::i("Batch" , "fetch video memoroy size : %d" , requestSizeResult);
    }
    ...
    void ShapeBatch::renderLinearGradientRect(Rect &rect, 
        ...
        if(!isDrawing_){
            Log::i("ShapeBatch" , "batch is not call begin()");
            return;
        }
    ...
    void SpriteBatch::doRender(unsigned int texId , float texWidth , float texHeight , 
                ...
                float cx , float cy , float angle){
        if(!isDrawing_){
            Log::i("SpriteBatch" , "batch is not call begin()");
            return;
        }
